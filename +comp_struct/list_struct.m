...
if nargin < 1; help comp_struct.list_struct; error('I / O error'); end
...
%			clean display - add index if needed
			...
			comp_struct.list_struct(getfield(s1(jj),char(fn1(ii))),v,n1p);
		end
	end
else
		...
		elseif iscell(s1)
			if isstruct(s1{1})
				for ii = 1:numel(s1)
					...
					fn1 = fieldnames(temp);
					for jj = 1:numel(fn1)
						comp_struct.list_struct(getfield(temp,char(fn1(jj))),v,[char(n1) '{' num2str(ii) '}.' char(fn1(jj))]);
					end
				end
			else
