...
classdef TEST__Stefan_solution < TESTCASE
    ...
    methods 
        ...
        function initialize(self)
            self.yml_const_file = 'CONSTANTS_YAML_STEFAN';
            % conduct parent class initialization
            initialize@TESTCASE(self)
        end

        
        function [t, Stefan_X] = get_Stefan_solution(self)
            ...
            % X = \alpha * sqrt(t)
            %
            % \alpha = 2 * \sqrt(\alpha_u) * \sqrt(Ste/2) * (1-Ste/8)
            % Ste = c_vu*Ts*(L)^-1
            %
            ...
            c_vu = (provider.CONST.c_w .* ground.STATVAR.waterIce(1) + ...
                    provider.CONST.c_m .* ground.STATVAR.mineral(1) + ...
                    provider.CONST.c_o .* ground.STATVAR.organic(1));
            k = ground.STATVAR.thermCond(1);
            alpha_u = k./c_vu;
            
            L = Lf * (theta-theta_u);
            Ste = c_vu.*Ts.*(1./L);
            alpha = 2.*sqrt(alpha_u).*sqrt(Ste/2).*sqrt(1-Ste/8);
            Stefan_X = alpha * sqrt(t);
            t = t/tile.CONST.day_sec;
        end
...
%             disp('=============================================')
        ...
        function plot(self)
            ...
            [Ste_t, Ste_X] = self.get_Stefan_solution();
            ...
            hold on
            h2 = plot(Ste_t, Ste_X, '-r', 'LineWidth', 1);
            set(gca, 'YDir','reverse')
            ylabel('Thaw depth [m]', 'Interpreter', 'none') 
            xlabel('Time [days]', 'Interpreter', 'none') 
            legend([h1, h2], {'Numerical', 'Analytical'}, 'Location', 'northeast')

            exportgraphics(gcf,'test_Stefan_solution.png','Resolution',300)
        end
