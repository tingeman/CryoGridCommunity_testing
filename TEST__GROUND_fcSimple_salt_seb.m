...
classdef TEST__GROUND_fcSimple_salt_seb < TESTCASE
    
    properties
        run_flag = true;     % Set this to false to skip the step that runs the model
        result_path = './results/';   % with trailing backslash
        run_name = 'test_GROUND_fcSimple_salt_seb';    % parameter file name and result directory 
    ...
    methods 
        ...
        function initialize(self)
            required_files = {'./forcing/Suossjavri_WRF_Norstore_daily_mean_extended_5000y.mat'};     % comma-separated cell array of files that must be present
